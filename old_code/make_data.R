...
for (current_team_index in 1:nrow(teams)) {
  ...
  for (current_year in 1999:2020) {
    for (current_week in 1:17) {
      ...
      general_data <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team) %>%
        select(posteam_type, div_game) %>%
        unique()
      total_off_plays <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, play_type == "pass" | play_type == "run") %>%
        count()
      total_def_plays <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, play_type == "pass" | play_type == "run") %>%
        count()

      # ----- OFFENSIVE EPA/GAME  AND FIRST DOWN RATE----- #
      current_off_rush_epa <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, !is.na(epa), penalty == 0, play_type == "run") %>%
        summarise(OFF_RUSH_EPA = mean(epa))
      current_off_pass_epa <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, !is.na(epa), penalty == 0, play_type == "pass") %>%
        summarise(OFF_PASS_EPA = mean(epa))
      current_first_downs_for <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, first_down == 1, play_type == "pass" | play_type == "run") %>%
        count()
      current_off_fdr <- current_first_downs_for / total_off_plays

      # ----- DEFENSIVE EPA/GAME ----- #
      current_def_rush_epa <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, !is.na(epa), penalty == 0, play_type == "run") %>%
        summarise(DEF_RUSH_EPA = mean(epa))
      current_def_pass_epa <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, !is.na(epa), penalty == 0, play_type == "pass") %>%
        summarise(DEF_PASS_EPA = mean(epa))
      current_first_downs_allowed <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, first_down == 1, play_type == "pass" | play_type == "run") %>%
        count()
      current_def_fdr <- current_first_downs_allowed / total_def_plays

      # ----- TURNOVER DIFFERENTIAL ----- #
      ## get giveaways
      current_giveaways <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, fumble_lost == 1 | interception == 1) %>%
        count()

      ## get takeaways
      current_takeaways <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, fumble_lost == 1 | interception == 1) %>%
        count()
      ...
      current_off_exp_plays <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, yards_gained >= 15, interception == 0 & fumble_lost == 0, penalty == 0, play_type == "pass" | play_type == "run") %>%
        count()
      current_off_exp_play_rate <- current_off_exp_plays / total_off_plays

      ## get defensive explosive plays allowed
      current_def_exp_plays <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, yards_gained >= 15, interception == 0 & fumble_lost == 0, penalty == 0, play_type == "pass" | play_type == "run") %>%
        count()
      ...
      # ----- POINT DIFFERENTIAL ----- #
      ## offensive points
      current_td_for <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, touchdown == 1, interception == 0 & fumble_lost == 0) %>%
        count()
      current_fg_for <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, field_goal_result == "made") %>%
        count()
      current_xp_for <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, extra_point_result == "good") %>%
        count()
      current_safety_for <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, safety == 1) %>%
        count()
      current_points_for <- (current_td_for * 6) + (current_fg_for * 3) + (current_safety_for * 2) + current_xp_for

      ## defensive points allowed
      current_td_allowed <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, touchdown == 1, interception == 0 & fumble_lost == 0) %>%
        count()
      current_fg_allowed <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, field_goal_result == "made") %>%
        count()
      current_xp_allowed <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, extra_point_result == "good") %>%
        count()
      current_safety_allowed <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, safety == 1) %>%
        count()
      ...
      current_off_penalties <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, penalty == 1 & penalty_team == current_team) %>%
        count()
      current_off_penalty_yds <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, penalty == 1 & penalty_team == current_team) %>%
        summarise(Off_Pen_Yds = sum(penalty_yards))
      current_def_penalties <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, penalty == 1 & penalty_team == current_team) %>%
        count()
      current_def_penalty_yds <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, penalty == 1 & penalty_team == current_team) %>%
      ...
      current_off_line_metric <- staley_pbp %>%
        filter(season == current_year, week == current_week, posteam == current_team, qb_hit == 1 | sack == 1) %>%
        count()
      current_def_line_metric <- staley_pbp %>%
        filter(season == current_year, week == current_week, defteam == current_team, qb_hit == 1 | sack == 1) %>%
        count()
