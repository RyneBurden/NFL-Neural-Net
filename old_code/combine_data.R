...
for (current_year in begin_year:end_year) {
    ...
    for (current_week in 1:num_weeks) {
        ...
        for (current_game_index in 1:nrow(current_week_schedule)) {
            ...
            # OAK to LV
            if (current_game$home_team == "OAK") {
                current_game$home_team <- "LV"
            }
            if (current_game$away_team == "OAK") {
                current_game$away_team <- "LV"
            }
            # STL to LA
            if (current_game$home_team == "STL") {
                current_game$home_team <- "LA"
            }
            if (current_game$away_team == "STL") {
                current_game$away_team <- "LA"
            }
            # SD to LAC
            if (current_game$home_team == "SD") {
                current_game$home_team <- "LAC"
            }
            if (current_game$away_team == "SD") {
                current_game$away_team <- "LAC"
            }
