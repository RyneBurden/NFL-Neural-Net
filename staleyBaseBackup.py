...
def train_network(network, trainingData, numEpochs, optimizer):
    ...
    #optimizer = torch.optim.Adam(network.parameters(), lr = .0005)
    # Variables to return
    running_loss = list()
    ...
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, .5)
...
def predict(row, model):
    ...
    # Variable to hold the probability of the result
...
def main():
    ...
    running_loss, networkState = train_network(model, trainingData, int(numEpochs), optimizer)
...
def graphLoss(numEpochs, running_loss):
    ...
    plt.annotate('Lowest loss: ' + str(running_loss[lowestLossIndex]) + ' @ epoch ' + str(lowestLossIndex), xy=(lowestLossIndex, running_loss[lowestLossIndex]), xytext=(int(numEpochs) / 2.5, 1.4), arrowprops=dict(arrowstyle="->", color="r"))
    ...
    #toSave = input("Would you like to save the network state? ")
        ...
        #torch.save(networkState, saveName)
